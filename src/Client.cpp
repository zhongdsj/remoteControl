...
int Client::sendMessage(unsigned int conv, std::string key, std::string message)
{
...
MyUdp* Client::connect(unsigned int conv, PCSTR ip, u_short port, boolean sendHand)
{
    // 构造远程地址
...
void Client::setOnMessageCallbackFunction(OnMessageCallbackFunction fun)
{
    this->m_onMessage = fun;
}

void Client::setOnConnectedCallbackFunction(OnConnectedCallbackFunction fun)
{
    this->m_onConnected = fun;
}

void Client::setOnClosedCallbackFunction(OnClosedCallbackFunction fun)
{
    this->m_onClosed = fun;
}
    ...
    while (this->m_alive)
    {
        ...
        for (auto pair : this->m_connected)
        {
            ...
            if(len > 0)
            {
                ...
                if(isMessage)
                {
                    const bool needResponse = this->parse(key, payload, res_key.get(), response.get());
                    if (needResponse)
                    {
...
bool Client::checkAndSendFind(unsigned int conv, MyUdp* zdp, User* user, unsigned int currentTime)
{
    // 计算时间差
...
bool Client::parse(char* _key, char* _payload, char* _res_key, char* _response)
{
    return this->m_onMessage(_key, _payload, _res_key, _response);
}
